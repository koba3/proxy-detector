...
/// iOS プラットフォーム向けのプロキシリポジトリ実装
/// 
/// ネイティブのiOS System Configuration frameworkを使用してプロキシ設定を取得します。
class IOSProxyRepository implements ProxyRepository {
  final MethodChannel _channel;

  /// コンストラクタ
  /// 
  /// [channelName] メソッドチャンネル名（デフォルト: 'proxy_detector'）
  IOSProxyRepository({String channelName = 'proxy_detector'})
      : _channel = MethodChannel(channelName);

  /// テスト用のコンストラクタ（カスタムチャンネルを注入可能）
  IOSProxyRepository.withChannel(MethodChannel channel)
      : _channel = channel;
